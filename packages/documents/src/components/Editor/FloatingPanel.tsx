import React, { useCallback, useEffect, useRef, useState, useMemo, createContext, useContext } from 'react';
...
export const FloatingPanel: React.FC<FloatingPanelProps> = ({
  ...
  const isDocked = position !== 'floating';
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const constrainedX = Math.max(0, Math.min(viewportWidth - currentSize.width, newX));
      const constrainedY = Math.max(0, Math.min(viewportHeight - currentSize.height, newY));
  ...
  const getFloatingPositionFromDocked = useCallback((dockedPosition: PanelPosition): FloatingPanelPosition => {
    ...
    switch (dockedPosition) {
      case 'left':
        return { 
          x: offset, 
          y: Math.max(offset, (window.innerHeight - height) / 2) 
        };
      case 'right':
        return { 
      ...
      case 'top':
        return { 
          x: Math.max(offset, (window.innerWidth - width) / 2), 
          y: offset 
        };
      case 'bottom':
        return { 
  ...
  const handlePositionChange = useCallback((newPosition: PanelPosition) => {
  ...
  const getPanelStyles = (): React.CSSProperties => {
    ...
    // Docked positions - all use fixed positioning now
    switch (position) {
      case 'left':
        return {
          ...baseStyles,
